...
REDIRECT_URI_TOKEN = f'http://{SERVER_HOST}:8080/user-token'
states = {}


def authorize_redirect(state, redirect_uri):
...
def get_token():
    ...
    if 'code' not in params:
        ...
        return redirect(authorize_redirect(state, REDIRECT_URI_TOKEN))
    token = get_access_token(params['code'], REDIRECT_URI_TOKEN)
    return f'Token to access user top tracks: {token}'
